...
unsigned int nTargetSpacing = 120;
...
const string strMessageMagic = "SportsCoin Signed Message:\n";
...
int64_t nSubsidy = 19800000 * COIN;
 return nSubsidy;
  }
...
static const int64_t nTargetTimespan = 500;  
//
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
   ...
   /*if (CheckStakeModifierCheckpoints(pindexNew->nHeight, pindexNew->nStakeModifierChecksum))
        printf("AddToBlockIndex() : Acccepted by stake modifier checkpoint height=%d, modifier=0x%016"PRIx64, pindexNew->nHeight, pindexNew->nStakeModifierChecksum);*/
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "SportsCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "SportsCoin 2016";
        CTransaction txNew;
        txNew.nTime = 1473670885;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime = 1473670885;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce = 810985;
		if(fTestNet)
        {
            block.nNonce = 1384077 ;
        }
		
		uint256 nhashMerkleRoot = uint256("0x3725ccad7cdad0ac7054d0e3283d1401f734e37dde926fffc0a41743f57712e8");
